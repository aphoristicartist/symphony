...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HNY} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HNY} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HNU} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HNU} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, HNQ} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, HNQ} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(HOE), fun((HOE) -> json())) -> json().
...
-spec array(list(HOI), fun((HOI) -> json())) -> json().
