...
-type response(HDP) :: {response, integer(), list({binary(), binary()}), HDP}.
...
-spec set_header(response(HEE), binary(), binary()) -> response(HEE).
...
-spec prepend_header(response(HEH), binary(), binary()) -> response(HEH).
...
-spec set_body(response(any()), HEM) -> response(HEM).
...
-spec try_map(response(HDQ), fun((HDQ) -> {ok, HDS} | {error, HDT})) -> {ok,
        response(HDS)} |
    {error, HDT}.
...
-spec map(response(HEO), fun((HEO) -> HEQ)) -> response(HEQ).
...
-spec expire_cookie(response(HEY), binary(), gleam@http@cookie:attributes()) -> response(HEY).
