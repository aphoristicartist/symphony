...
-type spec(IEM, IEN) :: {spec,
        IEM,
        integer(),
        integer(),
        fun((children(IEM)) -> children(IEN))}.

-opaque children(IEO) :: {ready, starter(IEO)} | {failed, child_start_error()}.

-opaque child_spec(IEP, IEQ, IER) :: {child_spec,
        fun((IEQ) -> {ok, gleam@erlang@process:subject(IEP)} |
            {error, gleam@otp@actor:start_error()}),
        fun((IEQ, gleam@erlang@process:subject(IEP)) -> IER)}.
...
-spec start_child(child_spec(any(), IEY, IEZ), IEY) -> {ok, child(IEZ)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    IFM,
    instruction(),
    child_spec(any(), IFM, IFO),
    child(IFO)
) -> {ok, {child(IFO), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(IGF), child_spec(any(), IGF, IGI)) -> children(IGI).
...
-spec add(children(IGN), child_spec(any(), IGN, IGQ)) -> children(IGQ).
...
-spec supervisor(
    fun((IGV) -> {ok, gleam@erlang@process:subject(IGW)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(IGW, IGV, IGV).
supervisor(Start) ->
...
-spec worker(
    fun((IHD) -> {ok, gleam@erlang@process:subject(IHE)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(IHE, IHD, IHD).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(IHL, IHM, any()),
    fun((IHM, gleam@erlang@process:subject(IHL)) -> IHS)
) -> child_spec(IHL, IHM, IHS).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), IHX)) -> gleam@otp@actor:init_result(state(IHX), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(IID)) -> gleam@otp@actor:next(message(), state(IID)).
            ...
            fun(Restarts) ->
                gleam@result:then(
                    ...
                    fun(_use0) ->
                        {Starter, _} = _use0,
                        {ok,
                            {state, Restarts, Starter, erlang:element(4, State)}}
                    end
                )
            end
        )
...
-spec loop(message(), state(III)) -> gleam@otp@actor:next(message(), state(III)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
