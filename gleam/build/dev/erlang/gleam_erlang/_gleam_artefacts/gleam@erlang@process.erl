...
-opaque subject(FUM) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FUM}.

-type do_not_leak() :: any().

-type selector(FUN) :: any() | {gleam_phantom, FUN}.
...
-type call_error(FUO) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FUO}.
...
-file("src/gleam/erlang/process.gleam", 166).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 186).
?DOC(
...
-spec select(selector(FVF), integer()) -> {ok, FVF} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-file("src/gleam/erlang/process.gleam", 195).
?DOC(
...
-spec select_forever(selector(FVJ)) -> FVJ.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-file("src/gleam/erlang/process.gleam", 204).
?DOC(
...
-spec map_selector(selector(FVL), fun((FVL) -> FVN)) -> selector(FVN).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-file("src/gleam/erlang/process.gleam", 213).
?DOC(
...
-spec merge_selector(selector(FVP), selector(FVP)) -> selector(FVP).
merge_selector(A, B) ->
    gleam_erlang_ffi:merge_selector(A, B).

-file("src/gleam/erlang/process.gleam", 257).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 229).
?DOC(
...
-spec selecting_trapped_exits(selector(FVT), fun((exit_message()) -> FVT)) -> selector(FVT).
...
-file("src/gleam/erlang/process.gleam", 267).
?DOC(
...
-spec selecting(selector(FVW), subject(FVY), fun((FVY) -> FVW)) -> selector(FVW).
...
-file("src/gleam/erlang/process.gleam", 130).
?DOC(
...
-spec selecting_record2(
    selector(FWB),
    any(),
    fun((gleam@dynamic:dynamic_()) -> FWB)
) -> selector(FWB).
...
-spec selecting_record3(
    selector(FWF),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FWF)
) -> selector(FWF).
...
-file("src/gleam/erlang/process.gleam", 317).
?DOC(
...
-spec selecting_record4(
    selector(FWJ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FWJ)
) -> selector(FWJ).
...
-file("src/gleam/erlang/process.gleam", 335).
?DOC(
...
-spec selecting_record5(
    selector(FWN),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FWN)
) -> selector(FWN).
...
-spec selecting_record6(
    selector(FWR),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FWR)
) -> selector(FWR).
...
-file("src/gleam/erlang/process.gleam", 371).
?DOC(
...
-spec selecting_record7(
    selector(FWV),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FWV)
) -> selector(FWV).
...
-file("src/gleam/erlang/process.gleam", 392).
?DOC(
...
-spec selecting_record8(
    selector(FWZ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FWZ)
) -> selector(FWZ).
...
-file("src/gleam/erlang/process.gleam", 442).
?DOC(
...
-spec selecting_anything(selector(FXD), fun((gleam@dynamic:dynamic_()) -> FXD)) -> selector(FXD).
selecting_anything(Selector, Handler) ->
    gleam_erlang_ffi:insert_selector_handler(Selector, anything, Handler).

-file("src/gleam/erlang/process.gleam", 466).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 473).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 513).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 524).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 539).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 558).
?DOC(
...
-spec deselecting_process_down(selector(FXS), process_monitor()) -> selector(FXS).
...
-file("src/gleam/erlang/process.gleam", 573).
?DOC(
...
-spec try_call(subject(FXV), fun((subject(FXX)) -> FXV), integer()) -> {ok, FXX} |
    {error, call_error(FXX)}.
...
-file("src/gleam/erlang/process.gleam", 614).
?DOC(
    " Send a message to a process and wait for a reply.\n"
    "\n"
    " If the receiving process exits or does not reply within the allowed amount\n"
...
-spec call(subject(FYC), fun((subject(FYE)) -> FYC), integer()) -> FYE.
...
-file("src/gleam/erlang/process.gleam", 643).
?DOC(
...
-spec try_call_forever(subject(FYK), fun((subject(FYM)) -> FYK)) -> {ok, FYM} |
...
-spec call_forever(subject(FYG), fun((subject(FYI)) -> FYG)) -> FYI.
...
-file("src/gleam/erlang/process.gleam", 681).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 688).
...
-file("src/gleam/erlang/process.gleam", 700).
?DOC(" Send a message over a channel after a specified number of milliseconds.\n").
-spec send_after(subject(FYT), integer(), FYT) -> timer().
...
-file("src/gleam/erlang/process.gleam", 724).
...
-file("src/gleam/erlang/process.gleam", 748).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 776).
?DOC(
    " Sends an exit signal to a process, indicating that the process is to shut\n"
...
-file("src/gleam/erlang/process.gleam", 804).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 815).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 838).
?DOC(
