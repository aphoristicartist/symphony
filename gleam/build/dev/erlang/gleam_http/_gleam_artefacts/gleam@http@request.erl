...
-spec set_header(request(GTY), binary(), binary()) -> request(GTY).
...
-spec prepend_header(request(GUB), binary(), binary()) -> request(GUB).
...
-spec set_body(request(any()), GUG) -> request(GUG).
set_body(Req, Body) ->
...
-spec map(request(GUI), fun((GUI) -> GUK)) -> request(GUK).
...
-spec set_query(request(GUU), list({binary(), binary()})) -> request(GUU).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(GUY), gleam@http:method()) -> request(GUY).
...
-spec set_scheme(request(GVF), gleam@http:scheme()) -> request(GVF).
...
-spec set_host(request(GVI), binary()) -> request(GVI).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(GVL), integer()) -> request(GVL).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(GVO), binary()) -> request(GVO).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(GVR), binary(), binary()) -> request(GVR).
...
-spec remove_cookie(request(GVW), binary()) -> request(GVW).
