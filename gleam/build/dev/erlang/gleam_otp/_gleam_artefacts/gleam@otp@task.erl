...
-opaque task(IQU) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:subject(IQU)}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message2(IQV, IQW) :: {m2_from_subject1, IQV} |
    {m2_from_subject2, IQW} |
    m2_timeout.

-type message3(IQX, IQY, IQZ) :: {m3_from_subject1, IQX} |
    {m3_from_subject2, IQY} |
    {m3_from_subject3, IQZ} |
    m3_timeout.

-type message4(IRA, IRB, IRC, IRD) :: {m4_from_subject1, IRA} |
    {m4_from_subject2, IRB} |
    {m4_from_subject3, IRC} |
    {m4_from_subject4, IRD} |
    m4_timeout.

-type message(IRE) :: {message, integer(), IRE} | message_timeout.
...
-spec try_await(task(IRJ), integer()) -> {ok, IRJ} | {error, await_error()}.
...
-spec await(task(IRN), integer()) -> IRN.
await(Task, Timeout) ->
...
-spec await_forever(task(IRR)) -> IRR.
await_forever(Task) ->
    assert_owner(Task),
    Selector = begin
        _pipe = gleam_erlang_ffi:new_selector(),
...
-spec try_await2_loop(
    gleam@erlang@process:selector(message2(ISB, ISC)),
    gleam@option:option({ok, ISB} | {error, await_error()}),
    gleam@option:option({ok, ISC} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, ISB} | {error, await_error()}, {ok, ISC} | {error, await_error()}}.
...
-spec try_await2(task(IRT), task(IRV), integer()) -> {{ok, IRT} |
        {error, await_error()},
    {ok, IRV} | {error, await_error()}}.
...
-spec try_await3_loop(
    gleam@erlang@process:selector(message3(ITC, ITD, ITE)),
    gleam@option:option({ok, ITC} | {error, await_error()}),
    gleam@option:option({ok, ITD} | {error, await_error()}),
    gleam@option:option({ok, ITE} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, ITC} | {error, await_error()},
    {ok, ITD} | {error, await_error()},
    {ok, ITE} | {error, await_error()}}.
...
-spec try_await3(task(ISQ), task(ISS), task(ISU), integer()) -> {{ok, ISQ} |
        {error, await_error()},
    {ok, ISS} | {error, await_error()},
    {ok, ISU} | {error, await_error()}}.
...
-spec try_await4_loop(
    gleam@erlang@process:selector(message4(IUO, IUP, IUQ, IUR)),
    gleam@option:option({ok, IUO} | {error, await_error()}),
    gleam@option:option({ok, IUP} | {error, await_error()}),
    gleam@option:option({ok, IUQ} | {error, await_error()}),
    gleam@option:option({ok, IUR} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, IUO} | {error, await_error()},
    {ok, IUP} | {error, await_error()},
    {ok, IUQ} | {error, await_error()},
    {ok, IUR} | {error, await_error()}}.
...
-spec try_await4(task(ITY), task(IUA), task(IUC), task(IUE), integer()) -> {{ok,
            ITY} |
        {error, await_error()},
    {ok, IUA} | {error, await_error()},
    {ok, IUC} | {error, await_error()},
...
-spec dict_to_list(gleam@dict:dict(integer(), IWH), integer(), IWH) -> list(IWH).
...
-spec try_await_all_loop(
    gleam@dict:dict(integer(), {ok, IVX} | {error, await_error()}),
    integer(),
    gleam@erlang@process:timer(),
    gleam@erlang@process:selector(message(IVX))
) -> list({ok, IVX} | {error, await_error()}).
...
-spec try_await_all(list(task(IVR)), integer()) -> list({ok, IVR} |
    {error, await_error()}).
