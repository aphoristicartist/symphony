...
-type message(HPM) :: {message, HPM} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(HPN, HPO) :: {continue,
        HPO,
        gleam@option:option(gleam@erlang@process:selector(HPN))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(HPP, HPQ) :: {ready, HPP, gleam@erlang@process:selector(HPQ)} |
    {failed, binary()}.

-type self(HPR, HPS) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        HPR,
        gleam@erlang@process:subject(HPS),
        gleam@erlang@process:selector(message(HPS)),
        gleam@otp@system:debug_state(),
        fun((HPS, HPR) -> next(HPS, HPR))}.

-type spec(HPT, HPU) :: {spec,
        fun(() -> init_result(HPT, HPU)),
        integer(),
        fun((HPU, HPT) -> next(HPU, HPT))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(HPV) :: {ack,
        {ok, gleam@erlang@process:subject(HPV)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 178).
-spec continue(HQC) -> next(any(), HQC).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(HQG, HQH), gleam@erlang@process:selector(HQG)) -> next(HQG, HQH).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(HQS))) -> gleam@erlang@process:selector(message(HQS)).
...
-spec receive_message(self(any(), HQO)) -> message(HQO).
...
-spec init_selector(
    gleam@erlang@process:subject(HUV),
    gleam@erlang@process:selector(HUV)
) -> gleam@erlang@process:selector(message(HUV)).
...
-spec initialise_actor(
    spec(any(), HRJ),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(HRJ)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), HRW)) -> {ok, gleam@erlang@process:subject(HRW)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(HSC, fun((HSD, HSC) -> next(HSD, HSC))) -> {ok,
        gleam@erlang@process:subject(HSD)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(HSJ), HSJ) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(HSL),
    fun((gleam@erlang@process:subject(HSN)) -> HSL),
    integer()
) -> HSN.
