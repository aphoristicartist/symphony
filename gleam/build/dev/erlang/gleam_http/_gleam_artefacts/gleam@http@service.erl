...
-spec map_response_body(
    fun((HIC) -> gleam@http@response:response(HHO)),
    fun((HHO) -> HHP)
) -> fun((HIC) -> gleam@http@response:response(HHP)).
...
-spec prepend_response_header(
    fun((HIG) -> gleam@http@response:response(HII)),
    binary(),
    binary()
) -> fun((HIG) -> gleam@http@response:response(HII)).
...
-spec ensure_post(gleam@http@request:request(HHT)) -> {ok,
        gleam@http@request:request(HHT)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(HJP)) -> HJX)) -> fun((gleam@http@request:request(HJP)) -> HJX).
